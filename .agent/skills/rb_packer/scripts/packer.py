...
def main():
    print("[PACKER] LLM Project Packer v3.0")
    print("=" * 50)
    ...
    if custom_includes:
        ...
        print(f"[INFO] Using custom includes: {', '.join(d.name for d in include_dirs)}")
    else:
        # Auto-detect
        include_dirs = detect_project_dirs(repo_root)
        print(f"[SCAN] Auto-detected directories: {', '.join(d.name for d in include_dirs)}")
    
    if not include_dirs:
        print("[WARN] No directories found to pack! Aborting.")
        return
    ...
    print(f"[FILE] Found {len(all_files)} file(s) to pack")
    ...
    central_dumps_dir = get_antigravity_root() / "_rb_dumps"
    ...
    print(f"[OK] Context dump created: {output_file.name} (Local: {dump_dir})")
    print(f"[OK] Copy sent to: {central_dumps_dir}")
    print(f"[STATS] Size: {file_size_kb:.1f} KB")
